...
class Group_Buying_Paypal_AP_Beta extends Group_Buying_Offsite_Processors {
	...
	protected function __construct() {
		...
		add_filter( 'group_buying_template_checkout/review-controls.php', array( __CLASS__, 'review_controls' ), 10 );
	}
	...
	private function get_payment( $payment, $deal_id, $amount, $qty = 1, $pre_app_key = '', $tracking_id = 0 ) {
		...
		if ( $is_share_percentage ) {
			...
			$secondary_share = ( $amount*$qty )*( $secondary_share_per*0.01 );
		...
		if ( !apply_filters( 'gb_paypal_ap_use_parallel_payments', FALSE ) ) {
			$fields['receiverList.receiver(0).amount'] = apply_filters( 'gb_paypal_ap_primary_receiver_amount', number_format( floatval( $amount ), 2 ) ); // since chained the entire amount is sent to the primary receiver
			...
			$fields['receiverList.receiver(0).amount'] = apply_filters( 'gb_paypal_ap_primary_receiver_amount', number_format( floatval( $subtotal ), 2 ) ); // since parallel the primary receiver gets the subtotal
			$fields['receiverList.receiver(0).primary'] = 'false';
		}

		// secondary payer
		$fields['receiverList.receiver(1).email'] = self::get_secondary( $deal_id );
		$fields['receiverList.receiver(1).amount'] = apply_filters( 'gb_paypal_ap_secondary_receiver_amount', number_format( floatval( $secondary_share ), 2 ) );
